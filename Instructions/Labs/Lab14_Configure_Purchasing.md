---
lab:
  title: 'Lab 14: Configure Purchasing'
  module: 'Set up Trade in Microsoft Dynamics 365 Business Central'
  duration: 60 minutes
  level: 300
  islab: true
---

## Exercise 14: Configure Purchasing
...
Vendor Cronus Cardoxy Sales is Contoso’s main supplier for all computer hardware items. They have different locations from where to order:
...
Cronus Cardoxy Sales guarantees to deliver all goods within 5 days, according to the ‘cost and freight’ shipment method.
...
1. Create an order address for the vendor
   ...
   2. On the **Vendors** list page, select vendor **Cronus Cardoxy Sales (V9002)**.
   ...
   6. In the opened order address card, fill in the following fields:
      ...
      2. In the **Name** field, enter the value **Cronus Cardoxy Sales**.
...
2. Specify receiving information
   1. On the vendor card for vendor **Cronus Cardoxy Sales**, go to the **Receiving** FastTab.
...
## Scenario
The computer hardware items are not manufactured by Contoso, but purchased through wholesalers, with Cronus Cardoxy Sales considered to be the main supplier.

Cronus Cardoxy Sales guarantees to deliver all goods within 5 days, however some special items may take a bit longer and have a deviating delivery time. For example, the 17” monitor will take up to 7 days to deliver.
...
1. Specify replenishment information
   ...
   4. In the item card, go to the **Replenishment** FastTab and fill in the following fields:
      ...
      4. In the **Purch. Unit of Measure** field, the value **PCS** is already filled in. Select on the Look Up Value button and select **+ New**.
      ...
      7. The new code is automatically filled in the **Purch. Unit of Measure** field on the item card.
...
## Scenario
Although Cronus Cardoxy Sales is considered the main supplier for all computer hardware items, these products can also be purchased with other vendors. For example, when the item is temporarily out of stock.
...
* All monitors can be purchased with vendor PURE-LOOK (V9005) and can be provided within 2 days:
  + PLM-345 = 15” 1501 FP Flat Panel
  + PLM-346 = 17” M780 Monitor
* All accessories can be purchased with vendor WoodMart Supply Co (V9006) and can be provided within 10 days:
...
* The total amount of the invoice is CAD 1,200.00, which is processed on the 73100 Health Insurance
...
You need to set up the recurring purchase lines for vendor OakvilleWorld (V9007), so that these lines are automatically filled in when creating a new purchase invoice.
...
1. Configure recurring purchase lines
   1. Select the Search icon in the top-right corner of the page, enter Vendors and then choose the related link.
   2. On the **Vendors** list page, select vendor **OakvilleWorld (V9007)**.
   ...
   5. On the new line, fill in the following fields:
      1. Select on the Look Up Value button in the **Code** field and select **+ New**.
      2. On the opened **Standard Purchase Code Card** page, fill in the following fields:
         ...
         4. In the **Lines** section, create a new line, and fill in the following fields:
            ...
            2. In the **No.** field, enter **73100**.In the **Quantity** field, enter **1**.
            3. Add the Amount Excl. Tax/VAT field:

Select on the **Settings** icon in the top-right corner and select **Personalize**.

Select **More** on the **Personalizing** ribbon.

Select the **Lines** area and select **+ Field** in the **Personalizing** ribbon.

An **Add field to page** pane will appear on the right side.

Select the **Amount Excl. Tax/VAT** field and drag it to the right of **Quantity** column in the **Lines** area

Select **Done** to close **Personalization**.

In the **Amount Excl. Tax/VAT** field, enter **300**. (25% of 1,200.00)

2. + - * 1. In the **Department Code** field, enter **SALES**.
      1. In the **Lines** section, create a new line, and fill in the following fields:
         1. In the **Type** field, select the **G/L Account** option.
         2. In the **No.** field, enter **73100**.
         3. In the **Quantity** field, enter **1**.
         4. In the Amount excl. Tax/VAT field, enter **420**. (35% of 1,200.00)
         5. In the **Department Code** field, enter **ADM**.
      2. In the **Lines** section, create a new line, and fill in the following fields:
         1. In the **Type** field, select the **G/L Account** option.
         2. In the **No.** field, enter **73100**.
         3. In the **Quantity** field, enter **1**.
         4. In the Amount excl. Tax/VAT field, enter **480**. (40% of 1,200.00)
         5. In the **Department Code** field, enter **PROD**.
      3. Select **OK** to close the **Standard Purchase Code Card** page.
    1. The new code is now automatically filled in in the **Code** field on the **Recurring Purchase Lines** page.
    2. In the **Insert Rec. Lines on Invoices** field, select the **Automatic** option.
  1. Close the **Recurring Purchase Lines** page