---
lab:
  title: 'Lab 15: Set Up Accounts Receivable'
  module: 'Work with Trade master data in Microsoft Dynamics 365 Business Central'
  duration: 50 minutes
  level: 300
  islab: true
---

## Exercise 15: Set Up Accounts Receivable
...
* Posted sales invoices are numbered based on the year and month in which they are posted. The first sales invoice, as of January 1st, 2026, will be posted with the number S26/01/0001. The next month will be S26/02/0001 and so on.
...
### Steps
1. Set up and assign number series.
   1. Select the Search icon in the top-right corner of the page, enter Sales & Receivables Setup, and then choose the related link.
   2. In the **Sales & Receivables Setup** page, go to the **Number Series** FastTab.
   3. In the **Customer Nos.** field, the value CUST is already filled in. Select on the Look Up Value button and select **+ New**.
   ...
   5. Select on the ellipses (…) in the top ribbon and then select **Lines**.
   ...
   8. In the opened **No. Series** page, select the number series **CUST**.
   9. Select the ellipses (…) and then select **Relationships**.
   ...
   15. In the **Posted Invoice Nos.** field, the value **S-INV+** is already filled in. Select on the Look Up Value button and then select **Select from full list.**
   16. In the opened **No. Series** page, the number series **S-INV+** is automatically selected. Choose **Show Details.**Select **Lines**.
   17. In the opened **No. Series Lines** page, select **+ New**.
       1. In the **Starting Date** field, enter **1/1/2026**.
       2. In the **Starting No.** field, enter **S26/01/0001**.
       3. In the **Increment-by No.** field, leave the value **1**.
   18. In the opened **No. Series Lines** page, select **+ New**.
       1. In the **Starting Date** field, enter **2/1/2026**.
       2. In the **Starting No.** field, enter **S26/02/0001**.
       3. In the **Increment-by No.** field, leave the value **1**.
   19. Close the **No. Series Lines** page.
   20. Close the **No. Series** page by selecting **OK**.
   21. The number series **S-INV+** remains filled in in the **Posted Invoice Nos.** field, in the **Sales & Receivables Setup** page.
2. Complete discount posting setup.
...
## Scenario
Based on the chart of accounts, you will set up the different customer posting groups. With their fast-growing business abroad, they want to keep track of their accounts receivables in the US (domestic) and in other EU countries (EU separately. A third group will be set up to register transactions with EU third-party alliances (intercompany) they have set up to expand their business even further.
...
### Steps
1. Set up a Customer Posting Group
   1. Select the Search icon in the top-right corner of the page, enter Customer Posting Groups, and then choose the related link.
   2. In the **Customer Posting Groups** page, select **+ New**.
   3. In the **Customer Posting Group Card** page, choose line EU in the following fields:
      1. In the **Receivables Account** field, fill in **10450**.
      2. Close the page.
   ...
   5. In the **Customer Posting Group Card** page, fill in the following fields:
      ...
      4. Close the page.
   6. From the **Customer Posting Groups** page, copy the other g/l accounts from the DOMESTIC customer posting group to the EU and **INTERCOMPANY** customer posting groups using F8. Copy the following fields:
...
2. Create and configure a new customer.
   ...
   6. In the window that opens, select the number series for web shop customers **CUST WEB** and select **OK**.
   7. Select **Yes**.
   8. In the **Name** field, enter **Generic EU web shop**.
   9. Select **Show more**.
   10. In the **Document Sending Profile** field, select on the Look Up Value button and select **+ New**.
   11. In the opened **Document Sending Profile** card page, fill in the following fields:
       ...
       6. Select **OK**.
   12. The new code is now automatically filled in the **Document Sending Profile** field on the customer card.
   13. Go to the **Invoicing** FastTab.
   14. In the **Gen. Bus. Posting Group** field, enter **EU**.
   15. In the **Customer Posting Group**, enter EU**.**
   16. Go to the **Payments** FastTab.
   17. In the **Payment Terms Code** field, select on the Look Up Value button and select **+ New**.
   ...
   21. In the **Payment Method Code** field, select on the Look Up Value button and select **+ New**.
   22. In the opened **Payment Methods** page, select **+ New** and fill in the following fields:
       ...
       4. Select the **Bal. Account No.** field, and then select **Select from full list**.
       5. On the **G/L Account List** page, select account number **20500/Credit Cards** and select **Edit**. Create a new account if you do not find it from the list.
       ...
       9. Select **OK**.
   23. The new code is now automatically filled in the **Payment Method Code** field on the customer card.
...
1. Create and export a configuration package.
   ...
   7. In the window that opens, select the **Customer** table(Table ID 18), and select **OK**.
   ...
   12. Select on **No.**
   13. Select on **Fields** again. On the field list page, first select **Clear Included**, to deselect the **Include Field** checkbox for all the fields.
...
2. Create and assign data template to the configuration package.
   ...
   5. In the **Configuration Templates** page, make sure the new template for domestic customers is selected, and select **OK**.
...
## Task 4: Set up cash receipt journal

## Scenario
The accounting manager at Contoso would now like to register checks from customers manually.

You must set up a cash receipt journal for this purpose.

### Steps
Create a general journal template batch.

1. In the **General Journal Templates** page, make sure **the  CASHRCPT** journal is selected.
  2. Select **Batches**.
  3. In the opened **General Journal Batches** page, select **+ New.**
  4. Fill in the following fields:
     1. In the **Name** field, enter**CHECKS.**
     2. In the **Description** field, enter **Manual Checks.**
     3. In the **Bal.Account type** select **Bank.**
     4. In the **Bal.Account No.** Enter **CHECKING.**